...
module.exports = async (client, m) => {
   try {
      ...
      if (m.chat.endsWith('broadcast')) await client.readMessages([m.key])
      if (m.chat.endsWith('broadcast') && m.mtype != 'protocolMessage') {
         let caption = `乂  *S T O R I E S*\n\n`
         if (/video|image/.test(m.mtype)) {
            caption += `${body ? body : ''}\n\n`
            caption += `*By : @${m.sender.replace(/@.+/, '')} (${client.getName(m.sender)})*`
      ...
      if (chats) {
         ...
         chats.chat += 1
         if (!m.isGroup && /conversation|extended/.test(m.mtype)) {
            chats.messages.push({
               msgId: chats.messages.length + 1,
               key: m.key,
               messageTimestamp: new Date * 1,
               message: m.message,
               mtype: m.mtype,
               chat: m.chat,
               sender: m.sender,
         ...
         } else if (!m.isGroup && !/conversation|extended/.test(m.mtype)) {
            chats.messages.push({
               msgId: chats.messages.length + 1,
               key: m.key,
               messageTimestamp: new Date * 1,
      ...
      // if (time == '00:00') setting.messages = []
      ...
      if (body && body.length != 1 && (isPrefix = (myPrefix || '')[0])) {
         ...
         try {
            ...
            global.db.chats[m.chat].lastchat = 0
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && global.blocks.some(no => m.sender.startsWith(no))) return client.updateBlockStatus(m.sender, 'block')
            if (setting.self && !['chatAI', 'exec', 'directly'].includes(name) && !isOwner && !m.fromMe) continue
