...
const { useSingleFileAuthState, DisconnectReason, makeInMemoryStore, fetchLatestBaileysVersion, msgRetryCounterMap } = require('@adiwajshing/baileys')
...
const { state } = useSingleFileAuthState(session, pino({
   level: 'silent'
}))
...
const connect = async () => {
   ...
   client.ev.on('group-participants.update', async (room) => {
      ...
      if (room.action == 'promote') client.reply(room.id, Func.texted('bold', `@${member.split`@`[0]} has been promoted.`))
      if (room.action == 'demote') client.reply(room.id, Func.texted('bold', `@${member.split`@`[0]} has been demoted.`))
      if (room.action == 'add') {
